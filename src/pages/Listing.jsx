    ...
    <main>
      ...
      >
        {listing.imgUrls.map((url, index) => (
          <SwiperSlide>
            <div
              className='relative w-full overflow-hidden h-[200px] md:h-[300px]'
              style={{
      ...
      >
        <FaShare class='text-lg text-slate-500' />
      </div>
      {shareLinkCopied && (
        <p class='fixed top-[23%] right-[5%] font-semibold border-2 border-gray-400 rounded-full bg-white z-10 p-2 '>
          Link Copied
        </p>
      )}
      ...
      <div className='m-4 flex flex-col md:flex-row max-w-6xl lg:mx-auto p-4 rounded-lg shadow-lg bg-white lg:space-x-5'>
        <div className=' w-full '>
          ...
          <p class='flex items-center mt-6 mb-3 font-semibold'>
            <FaMapMarkerAlt class='text-green-700 mr-1' />
            {listing.address}
          </p>
          ...
          <ul class='flex items-center space-x-2 sm:space-x-10 text-sm font-semibold mb-6'>
            <li class='flex items-center whitespace-nowrap'>
              <FaBed class='text-lg mr-1' />
              {listing.bedrooms > 1 ? `${listing.bedrooms} Beds` : '1 Bed'}
            </li>
            <li class='flex items-center whitespace-nowrap'>
              <FaBath class='text-lg mr-1' />
              {listing.bedrooms > 1 ? `${listing.bathrooms} Baths` : '1 Bath'}
            </li>
            <li class='flex items-center whitespace-nowrap'>
              <FaParking class='text-lg mr-1' />
              {listing.parking ? 'Parking Spot' : 'No Parking'}
            </li>
            <li class='flex items-center whitespace-nowrap'>
              <FaChair class='text-lg mr-1' />
          ...
          {listing.userRef !== auth.currentUser?.uid && !contactOwner && (
            <div className='mt-6 '>
              <button
                ...
                class='px-7 py-3 bg-blue-600 text-white font-medium text-sm uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg w-full text-center transition duration-150 ease-in-out'
              >
                Contact Owner
              </button>
            </div>
        ...
        <div className='w-full h-[200px] md:h-[400px] z-10 mt-6 md:mt-0 md:ml-2 '>
          <MapContainer
